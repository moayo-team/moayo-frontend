...
export const useProfileData = () => {
  ...
  const mappedExperiences = useMemo(() => {
    ...
    return res.map((exp: ExperienceSummary) => ({
      ...
      endDate: exp.endDate,
      period: `${(exp.startDate || "").replace(/-/g, ".")} - ${(exp.endDate || "").replace(/-/g, ".")}`, participation: exp.activity,
      intro: exp.summary || "",
      visible: exp.visible,
...
// 공개 이력 파일 조회 
export const usePublicExperienceFiles = (experienceId: number | null) => {
  return useQuery({
...
//공개 이력 링크 조회 
export const usePublicExperienceLinks = (experienceId: number | null) => {
  return useQuery({
...
// 특정 사용자 공개 이력 상세 조회 
export const usePublicExperienceDetail = (experienceId: number | null) => {
  return useQuery({
